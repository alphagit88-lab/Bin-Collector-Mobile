...
const SupplierRequestsScreen: React.FC = () => {
  ...
  const fetchPendingRequests = useCallback(async () => {
    try {
      const response = await api.get<{requests: Job[]}>(
        ENDPOINTS.BOOKINGS.PENDING,
      );
      ...
      Alert.alert(
        'Error',
        `Failed to fetch pending requests: ${error instanceof Error ? error.message : 'Network error'}`,
      );
    } finally {
      setLoading(false);
      setRefreshing(false);
